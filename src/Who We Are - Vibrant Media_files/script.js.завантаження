...
$(document).ready(function(){
    ...
    if ((url.indexOf("/en/") >= 1)||(url.indexOf("/de/") >= 1)){
        ...
        (function() {
            var hn = document.createElement('script'); hn.type = 'text/javascript'; hn.async = true; hn.setAttribute('data-ev-hover-pid', 1830); hn.setAttribute('data-ev-hover-ocid', 179);
    ...
    $('#cs-filters .filter').click(function() {
        var filterValue = $(this).attr('data-filter');
        $('#case_studies').isotope({ filter: filterValue });
    ...
    /*UBER NAV*/
    $('.header-nav .menu-item').mouseover(
        function(){
            $this = $(this);
            $navItem = $this.find('a');
            $navItemText = $navItem.html();
            $navItemText = $navItemText.toLowerCase();
            $selectedMenu = $navItemText.replace(/[^a-zA-Z0-9]+/g,'-');
			
			if (console) {
				console.debug($selectedMenu);
			}
			
            if($('.'+$selectedMenu).hasClass('no-children')){
            ...
            $('.'+$selectedMenu).addClass('active-menu');
        }
    );
    ...
    $('.more.icon').click(function(){
        ...
        $('.children.'+$slug).toggleClass('active');
    });
});
